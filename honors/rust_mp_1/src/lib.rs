...
impl Dataset {
    ...
    pub fn predict(&self, input: &Vec<f64>) -> String {
        let height = self.matrix.len(); // number of rows
        let mut closest = 0;
        let mut closest_dist = f64::MAX;
        for i in 0..height {
            let dist = distance(input, &self.matrix[i]);
            if dist < closest_dist {
                closest = i;
                closest_dist = dist;
            }
        }
        self.labels[closest].clone()
    }
}
...
pub fn normalize(data: &mut Vec<Vec<f64>>, means: &Vec<f64>, stds: &Vec<f64>) {
    let height = data.len(); // number of rows
    let width = data[0].len(); // number of cols
    for i in 0..width {
        for j in 0..height {
...
pub fn mean(data: &Vec<Vec<f64>>) -> Vec<f64> {
    ...
    for i in 0..width {
        means.push(0f64);
        for j in 0..height {
            means[i] += data[j][i] / (height as f64);
        }
    }
    means
}
...
pub fn std(data: &Vec<Vec<f64>>, means: &Vec<f64>) -> Vec<f64> {
    ...
    for i in 0..width {
        let mut sum = 0f64;
        for j in 0..height {
            sum += (data[j][i] - means[i]).powf(2.0);
        }
        stds.push(f64::sqrt(sum / (height as f64)));
    }
    stds
}
